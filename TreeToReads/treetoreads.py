...
class TreeToReads(object):
    ...
    def __init__(self, configfi, run=1, main=None):
        ...
        self.configfi = configfi										#configure file
        self.run = run
        self.main = main
    ...
    def test_deps(self):
        ...
        if call(['which', 'samtools'], stdout=open('/dev/null', 'w')) == 1:
            ...
            self.no_sam = 0 #just trying to get this to work idk
        else:
            self.no_sam = 0

    def read_args(self):												#input arguments from seqsim
...
#        sys.stdout.write("scaler is {}\n".format(self.seqgen_scaler))
    ...
    def run_art(self):
		...
		#but we didn't even make this a sam yet???
                # LK - sam => bam
